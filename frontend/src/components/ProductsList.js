...
class Articles extends Component {
  componentWillMount(){
    /*if(this.props.user_id===undefined){
      axios.get('https://backend-pipemax85.c9users.io/articles').then(response => this.setState({articles: response.data}));
    }else{
      axios.get('https://backend-pipemax85.c9users.io/belongsuser?user_id='+1).then(response => this.setState({articles: response.data}));
    }*/
    ...
    this.state.messages =   JSON.parse(JSON.stringify(eval("(" + "[]" + ")")));
  }
  constructor(props) {
    super(props);
    this.state = { articles: []} ;
