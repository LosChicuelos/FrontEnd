...
class SearchList extends Component {
  ...
  bufferArticles(){
      axios.get('http://127.0.0.1:3060/filter?page_size=20&&name='+this.props.filter.search+"&&min_price="+this.props.filter.min_price+"&&max_price="+this.props.filter.max_price+"&&qualification="+this.props.filter.qualification).then(response => this.setState({articles: response.data, status: response.status, search: this.props.filter}));
