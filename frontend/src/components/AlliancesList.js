  ...
  componentWillMount(){
    ...
    this.state.messages =   JSON.parse(JSON.stringify(eval("(" + str + ")")));*/
      ...
      if(this.props.parametros.includes("confirmyes")){
        axios.get('https://backend-bsdiaza.c9users.io/alliancesbelongsuser?confirmyes=si&user_id='+this.props.user_id).then(response => 
            {
        ...
        axios.get('https://backend-bsdiaza.c9users.io/alliancesbelongsuser?confirmno=si&user_id='+this.props.user_id).then(response => 
            {
                console.log(response.data);
                this.setState({messages: response.data})
            }
